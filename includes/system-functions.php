...
function BuildPageTabs()
{
	...
	if ($userbank->is_admin())
		AddTab("<i class='zmdi zmdi-star zmdi-hc-fw'></i> Admin-Panel", "index.php?p=admin", "Panel for administrators. Management of servers, administrators, settings.");
		...
		if(($userbank->HasAccess(ADMIN_OWNER)) && ($GLOBALS['config']['page.vay4er'] == "1"))
			$submenu->addMenuItem("Promokeys", 0,"", "index.php?p=admin&amp;c=pay_card", true);
		...
		if($userbank->HasAccess( ADMIN_OWNER|ADMIN_ADD_BAN|ADMIN_EDIT_OWN_BANS|ADMIN_EDIT_ALL_BANS))
			$submenu->addMenuItem("Mutes and Gags", 0,"", "index.php?p=admin&amp;c=comms", true);
		...
		if($userbank->HasAccess(ADMIN_OWNER))
			$submenu->addMenuItem("Menu", 0,"", "index.php?p=admin&amp;c=menu", true);
...
function BuildBreadcrumbs()
{
	...
	if(isset($_GET['c']))
	{
		...
		{
			case "admins":
				$cat = "Admins management";
				break;
			case "servers":
				$cat = "Servers management";
				break;
			case "bans":
				$cat = "Bans management";
				break;
			case "comms":
				$cat = "Mutes and Gags management";
				break;
			case "groups":
				$cat = "Groups management";
				break;
			case "settings":
	...
	if($GLOBALS['config']['page.xleb']){
		...
		if(!empty($bread))
			...
			$text = "<li><a href='index.php?p=home'>Home</a></li>";
...
function SmFlagsToSb($flagstring, $head=true)
{
	...
	$string = "";
	if($head)
		$string .= "<p class='c-blue'>Server permissions</p><ul class='clist clist-star'>";
	if(empty($flagstring))
		{
			$string .= "<li><i>No permissions</i></li>";
			return $string;
		}
	if((strstr($flagstring, "a") || strstr($flagstring, "z")))
		$string .= "<li> Reserved slots</li>";
	if((strstr($flagstring, "b") || strstr($flagstring, "z")))
		$string .= "<li> Generic admin</li>";
...
function SecondsToString($sec, $textual=true)
{
	if($textual)
	{
		...
		$desc = array('month','week','day','hour','min','sec');
		$ret = null;
...
function getReasonByCode($code, $frmt) {
        switch ($code) {
                case 1:      return "The file has exceeded the allowed size";
                case 2:      return "The file has exceeded the allowed size";
                case 3:      return "The file was only partially received";
                case 4:      return "The file was not uploaded";
                case 6:      return "There is no temporary download folder";
                case 7:      return "No write rights";
                case 8:      return "PHP extension forcibly stopped file upload";
                case 100500: return "The file must be in the format ".$frmt;
...
function generateMsgBoxJS($title = "Успех!", $text = "Action completed successfully", $color = "green", $redirect = "", $button = true) {
...
function smtpmail($mail_to, $subject, $message, $headers='') {
    ...
    if (!server_parse($socket, "250", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "Unable to send HELO");
        fclose($socket);
        return false;
    }
    ...
    if (!server_parse($socket, "334", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "Can't find an answer to the client authorization request.");
        fclose($socket);
        return false;
    }
    ...
    if (!server_parse($socket, "334", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "The username was not accepted by the server.");
        fclose($socket);
        return false;
    }
    ...
    if (!server_parse($socket, "235", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "The password was not accepted by the server as correct.");
        fclose($socket);
        return false;
    }
    ...
    if (!server_parse($socket, "250", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "Unable to send command.");
        fclose($socket);
        return false;
    }
    ...
    @fputs($socket, $SEND."\r\n.\r\n");
    if (!server_parse($socket, "250", __LINE__)) {
        new CSystemLog("e", "SMTP Mailing Error", "Can't send email to remote server.");
        fclose($socket);
        return false;
    }
