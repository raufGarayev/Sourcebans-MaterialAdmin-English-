...
class CErrorHandler {
    ...
    private static function DrawErrorMessage($message, $function = null, $title = "System error") {
        global $theme;
    ...
    public static function BasicErrorCatcher($errno, $errstr, $errfile, $errline) {
        ...
        switch ($errno) {
            case E_USER_ERROR:
                $msg = "[$errno] $errstr<br />\n";
                $msg .= "Fatal error on line $errline at file $errfile";
            ...
            case E_USER_WARNING:
                $msg = "[$errno] $errstr<br />\n";
                $msg .= "Error on line $errline at file $errfile";
                $GLOBALS['log']->AddLogItem("w", "PHP Warning", $msg);
                break;

            case E_USER_NOTICE:
                $msg = "[$errno] $errstr<br />\n";
                $msg .= "Notice on line $errline at file $errfile";
    ...
    public static function FatalErrorCatcher() {
        ...
        if (in_array($error['type'], self::$fatalcodes)) {
            self::CloseOutputBuffer(false);
            self::DrawErrorMessage("Fatal error PHP\n" . $error['message'] . "\n\n" . $error['file'] . "::" . $error['line'], null, "Crytic error PHP");
        }
    }
}
?>
