...
function DoLogin(redir)
{
	...
	{
		$('loginUsername.msg').setHTML('You need to enter username!');
	...
	{
		$('loginPassword.msg').setHTML('You need to enter password!');
...
{
	if(archiv == '2') {
		var noPerm = confirm("Are you sure you want to restore the player's ban request '" + name + "' from archieve?");
	}
	else if(archiv == '1') {
		var noPerm = confirm("Are you sure you want to transfer the request to the player's ban '" + name +"' to archieve?");
	}
	else {
		var noPerm = confirm("Are you sure you want to delete the player's ban request '" + name +"'?");
	}
	if(noPerm == false)
		return;
...
{
	if(archiv == '2') {
		var noPerm = confirm("Are you sure you want to restore the player's ban protest '" + name + "' from archieve?");
	}
	else if(archiv == '1') {
		var noPerm = confirm("Are you sure you want to postpone the player's ban protest '" + name +"' to archieve?");
	}
	else {
		var noPerm = confirm("Are you sure you want to remove the player's ban protest '" + name +"'?");
	}
	if(noPerm == false)
	{
		return;
...
{
	var noPerm = confirm("Are you sure you want to remove server: '" + name +"'?");
	if(noPerm == false)
	{
		return;
	}
...
{
	if(confirm==0) {
		ShowBox('Remove ban', '<p class="c-black m-t-20 f-14">Are you sure to remove ban of '+(bulk=="true"?"choosed(ых)":"")+' '+(bulk=="true"?"players":"\'"+ name +"\'")+'?</p>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="RemoveBan(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'1\''+(bulk=="true"?", \'true\'":"")+');document.getElementById(\'rban\').disabled = true;" name="rban" class="btn btn-lg btn-primary waves-effect" id="rban" value="Remove ban" />');
	} else if(confirm==1) {
		if(page != "") 
			var pagelink = page;
		else
...
{
	if(popup==1) {
		ShowBox('Unban', '<div class="form-group has-warning has-feedback"><label class="control-label f-14" for="inputWarning2">Please write a short comment why you are going to unban '+(bulk=="true"?"players":"player \'"+ name +"\'")+'!</label><div class="fg-line"><input type="text" class="form-control" id="inputWarning2" name="ureason"></div><span class="zmdi zmdi-alert-triangle form-control-feedback"></span><p class="help-block" id="ureason.msg"></p></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="if (UnbanBan(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'0\''+(bulk=="true"?", \'true\'":"")+')) document.getElementById(\'uban\').disabled = true;" name="uban" class="btn btn-lg btn-primary waves-effect" id="uban" value="Unban" />');
	} else if(popup==0) {
		if(page != "") 
			var pagelink = page;
		else
...
function ProcessEditAdminPermissions()
{
	...
	if($('immunity'))
	{
	 	if(IsNumeric($('immunity').value))
			...
			ShowBox("Error", "Immunity value must be in numbers (0-9)", "red", "", true);
	}else
		xajax_EditAdminPerms(aid, Mask, srvMask);
}
...
function ProcessEditGroup(type, name)
{
	...
	if(name == "")
	{
		ShowBox("Error", "You need to enter group name.", "red", "", true);
		$('groupname.msg').innerHTML = 'Type group name.';
...
function process_edit_server()
{
    ...
    {
        $('rcon2.msg').innerHTML = 'Passwords does not match.';
...
function icon(name)
{
	$('icon.msg').setHTML("Loaded: <b>" + name + "</b>");
	icname = name;
	if($('icon_hid'))
		$('icon_hid').value = name;
}
function ProcessMod()
{
	...
	if(!$('name').value)
	{
		$('name.msg').setHTML('You must enter a name for the generated MOD.');
	...
	if(!$('folder').value)
	{
		$('folder.msg').setHTML('You must enter name of mod folder.');
...
function CheckEmail(type, id)
{
	var err = 0;
	if($('subject').value == "") {
		$('subject.msg').setHTML("You need to enter subject for sending e-mail.");
	...
	if($('message').value == "") {
		$('message.msg').setHTML("You need to enter message for sening e-mail.");
...
function ClearLogs()
{
	var noPerm = confirm("Are you sure you want to delete all log entries?");
	if(noPerm == false)
	{
		return;
	}
...
{
	var noPerm = confirm("Are you sure you want to deleted '" + name +"'?");
	if(noPerm == false)
		return;
	xajax_RemoveMod(id);
}
...
{
	//ShowBox('Ban added', 'Ban successfully added<br><iframe id="srvkicker" frameborder="0" width="100%" src="pages/admin.kickit.php?check='+check+'&type='+type+'"></iframe>', 'green', 'index.php?p=admin&c=bans', true);
	ShowBox('Ban added', 'Ban successfully added<br><iframe id="srvkicker" frameborder="0" width="100%" src="pages/admin.kickit.php?check='+check+'&type='+type+'"></iframe>', 'green', '', false);
}
...
function ShowRehashBox(servers, title, msg, color, redirUrl = '', appendText = true)
{
	...
	if(servers == '')
	{
		...
		if (appendText)
			msg = msg + '<br /><hr /><i>Refresh admin and group information for all linked servers...</i>';
...
function ShowRehashBox_pay(servers, title, msg, color, redir, card)
{
	...
	if(servers == '')
	{
		...
		msg = msg + '<br /><hr /><i>Refresh admin and group information for all linked servers...</i><div id="rehashDiv" name="rehashDiv" width="100%"></div>';
		ShowBox(title, msg, color, '', false);
		$('dialog-control').setStyle('display', 'none');
		xajax_RehashAdmins_pay(servers, 0, card);
	}
...
function ProcessComment()
{
	...
	{
		$('commenttext.msg').setHTML('Enter comment');
...
{
	var checkUp = confirm("Are you sure you want to delete this comment?");
	if(checkUp == false)
		return;
...
function TickSelectAll()
{
	...
	if($('tickswitch').value==0) {
		$('tickswitch').value=1;
		$('tickswitch').setProperty('title','Remove all');
		$('tickswitchlink').addClass('alert-success');
		$('tickswitchlink').innerHTML = 'All bans on the current page have been highlighted.';
		$('tickswitchlink_1').innerHTML = 'Select all bans on the current page.';
		$('tickswitchlink').style.display = 'block';
		setTimeout("$('tickswitchlink').style.display = 'none';", 2500);
	} else {
		$('tickswitch').value=0;
		$('tickswitch').setProperty('title','Choose all');
		$('tickswitchlink').addClass('alert-success');
		$('tickswitchlink').innerHTML = 'Ban highlighting on the current page has been removed.';
		$('tickswitchlink_1').innerHTML = 'Unselect bans on the current page.';
		$('tickswitchlink').style.display = 'block';
		setTimeout("$('tickswitchlink').style.display = 'none';", 2500);
	}
}
...
function BulkEdit(action, bankey)
{
	...
	{
		case "U":
			UnbanBan(ids, bankey, "", "Unban all", "1", "true");
		break;
		case "D":
			RemoveBan(ids, bankey, "", "Delete all", "0", "true");
		break;
	}
}
...
{
	var checkUp = confirm("Are you sure you want to ban all friends of player '"+name+"'?");
	if(checkUp == false)
		return;
	ShowBox("Banning friends of "+name, "Banning all friends of player '"+name+"'.<br />Wait...<br />it can take a very long time, it all depends on the number of his friends.", 'blue', '', true);
	$('dialog-control').setStyle('display', 'none');
...
{
	if(popup==1) {
		ShowBox('Send message', '<b>Please enter the message you want to send <br>\''+name+'\'.</b><br>The server must be enabled basechat.smx<br><i>&lt;sm_psay&gt;</i>.<br><textarea rows="3" cols="40" name="ingamemsg" id="ingamemsg" style="overflow:auto;"></textarea><br><div id="ingamemsg.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" name="ingmsg" class="btn btn-lg btn-primary waves-effect" onmouseover="ButtonOver(\'ingmsg\')" onmouseout="ButtonOver(\'ingmsg\')" id="ingmsg" value="Send" />');
		...
		if(message == "") {
			$('ingamemsg.msg').setHTML("Please enter message.");
		...
		$('dialog-control').setStyle('display', 'none');
		ShowBox('Send Message', 'Sending request...', 'blue', '', false);
...
{
	if(conf==0)	{
		ShowBox('Кик игрока', '<b>Are you sure you want to kick player  <br>\''+name+'\'?</b>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" name="kbutton" class="btn btn-lg btn-primary waves-effect" onmouseover="ButtonOver(\'kbutton\')" onmouseout="ButtonOver(\'kbutton\')" id="kbutton" value="Yes" /> ');
		$('dialog-control').setStyle('display', 'inline-block');
...
{
    ShowBox('Viewing Steam profile', 'Creating Steam Community Profile Link"'+name+'", wait...', 'blue', '', false);
    $('dialog-control').setStyle('display', 'none');
...
{
	if(confirm==0) {
		ShowBox('Remove block', 'Are you sure you want to remove block of '+ name + '?', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="RemoveBlock(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'1\''+');document.getElementById(\'rban\').disabled = true;" name="rban" class="btn btn-lg btn-primary waves-effect" id="rban" value="Remove block" />');
	} else if(confirm==1) {
		if(page != "") 
			var pagelink = page;
		else
...
{
	if(popup==1) {
		ShowBox('Reason chat unblock', '<b>Please leave a short comment why you want to enable chat for the player '+"\'"+ name +"\'"+'!</b><br><textarea rows="3" cols="40" name="ureason" id="ureason" style="overflow:auto;"></textarea><br><div id="ureason.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="UnGag(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'0\''+');" name="uban" class="btn btn-lg btn-primary waves-effect" onmouseover="ButtonOver(\'uban\')" onmouseout="ButtonOver(\'uban\')" id="uban" value="Unblock chat" />');
	} else if(popup==0) {
		if(page != "")
			var pagelink = page;
		else
		...
		reason = $('inputWarning2').value;
		if(reason == "") {
			$('ureason.msg').setHTML("Leave a comment.");
			$('ureason.msg').setStyle('display', 'block');
			return;
		} else {
			$('ureason.msg').setHTML('');
...
{
	if(popup==1) {
		ShowBox('Reason voice unblock', '<b>Please leave a short comment as to why you want to turn the microphone on for the player '+"\'"+ name +"\'"+'!</b><br><textarea rows="3" cols="40" name="ureason" id="ureason" style="overflow:auto;"></textarea><br><div id="ureason.msg" class="badentry"></div>', 'blue', '', true);
		$('dialog-control').setHTML('<input type="button" onclick="UnMute(\''+id+'\', \''+key+'\', \''+page+'\', \''+addslashes(name.replace(/\'/g,'\\\''))+'\', \'0\''+');" name="uban" class="btn btn-lg btn-primary waves-effect" onmouseover="ButtonOver(\'uban\')" onmouseout="ButtonOver(\'uban\')" id="uban" value="Unblock voice" />');
	} else if(popup==0) {
		if(page != "")
			var pagelink = page;
		else
...
{
	ShowBox('Block added', 'Block successfully added<br><iframe id="srvkicker" frameborder="0" width="100%" src="pages/admin.blockit.php?check='+check+'&type='+type+'&length='+length+'"></iframe>', 'green', 'index.php?p=admin&c=comms', true);
}

function removeExpiredAdmins() {
	if(confirm("Delete all expired admins?")) {
		if(!confirm("Are you sure? All expired admins will be deleted!")) { return false; }
	}else{ return false; }
