...
function set_counter(count)
{
	...
	if(srvcount==-{$total}- || count=='-1') {
		//parent.document.getElementById('dialog-control').innerHTML = "<font color=\"green\" class=""><b>Search is over. Redirecting...</b></font>"+parent.document.getElementById('dialog-control').innerHTML;
		parent.document.getElementById('dialog-control').innerHTML = "<font color=\"green\" class=\"f-15\"><b>Search is over. Redirecting...</b></font>"+parent.document.getElementById('dialog-control').innerHTML;
...
<h3 style="font-size: 12px;">Searching for violator on servers....</h3>
<table border="0">
-{foreach from=$servers item=serv}-
<tr>
	<td><div id="srvip_-{$serv.num}-"><font size="1">-{$serv.ip}-:-{$serv.port}-</font></div></td>
	<td>
		<div id="srv_-{$serv.num}-"><font size="1">Wait...</font></div>
	</td>
</tr>
-{/foreach}-
</table>
