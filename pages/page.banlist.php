...
if (isset($_GET['a']) && $_GET['a'] == "unban" && isset($_GET['id']))
{
	if ($_GET['key'] != $_SESSION['banlist_postkey'])
		die("Possible hacking attempt (URL key missmatch)");
	...
	foreach($bids AS $bid) {
		...
		if(empty($row) || !$row) {
			$fail++;
			if(!isset($_GET['bulk'])) {
				echo "<script>setTimeout('ShowBox(\"Player is not unbanned\", \"Player is not unbanned. Or he is unbanned before, or ban was invalid.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=banlist$pagelink'\", false);', 1350);</script>";
				PageDie();
			}
			continue;
		}
		...
		if($res){
			if(!isset($_GET['bulk']))
				echo "<script>setTimeout('ShowBox(\"Player unbanned\", \"<b>".StripQuotes($row['name'])."</b> (<b>" . ($row['type']==0?$row['authid']:$row['ip']) . "</b>) unbanned.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"green\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
			$log = new CSystemLog("m", "Player unbanned", "'".StripQuotes($row['name'])."' (" . ($row['type']==0?$row['authid']:$row['ip']) . ") has been unbanned");
			$ucount++;
		}else{
			if(!isset($_GET['bulk']))
				echo "<script>setTimeout('ShowBox(\"Player is not unbanned\", \"Error <b>".StripQuotes($row['name'])."</b><br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
			$fail++;
		}
	}
	if(isset($_GET['bulk']))
		echo "<script>setTimeout('ShowBox(\"Player unbanned\", \"$ucount unbanned.<br>$fail failed.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"green\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
}
else if(isset($_GET['a']) && $_GET['a'] == "delete")
{
	...
	{
		echo "<script>setTimeout('ShowBox(\"Error\", \"You do not have access for this.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
		PageDie();
	}
	...
	foreach($bids AS $bid) {
		...
		if($res){
			if(!isset($_GET['bulk']))
				echo "<script>setTimeout('ShowBox(\"Ban deleted\", \"Ban of <b>".StripQuotes($steam['name'])."</b> (<b>".($steam['type']==0?$steam['authid']:$steam['ip'])."</b>) deleted from SourceBans<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"green\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
			$log = new CSystemLog("m", "Ban Deleted", "Ban ".StripQuotes($steam['name'])."' (" . ($steam['type']==0?$steam['authid']:$steam['ip']) . ") has been deleted.");
			$dcount++;
		}else{
			if(!isset($_GET['bulk']))
				echo "<script>setTimeout('ShowBox(\"Ban is not deleted\", \"Deleting ban of <b>".StripQuotes($steam['name'])."</b> failed.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
			$fail++;
		}
	}
	if(isset($_GET['bulk']))
		echo "<script>setTimeout('ShowBox(\"Players deleted\", \"$dcount players removed from SourceBans.<br>$fail failed.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"green\", \"index.php?p=banlist$pagelink\", false);', 1350);</script>";
}
...
while (!$res->EOF)
{
	...
	$data['ban_length'] = $res->fields['ban_length'] == 0 ? 'Permanently' : SecondsToString(intval($res->fields['ban_length']));
...
//	}
	...
	if($alrdybnd->fields['count']==0)
		$data['reban_link'] = CreateLinkR('Reban',"index.php?p=admin&c=bans".$pagelink."&rebanid=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']."#^0");
	else
		$data['reban_link'] = false;
	$data['blockcomm_link'] = CreateLinkR('Silence',"index.php?p=admin&c=comms".$pagelink."&blockfromban=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']."#^0");
	...
	$data['friend_ban_link'] = CreateLinkR('Ban friends', '#', '', '_self', false, "BanFriendsProcess('".$data['communityid']."','".StripQuotes($data['player'])."');return false;");
	$data['edit_link'] = CreateLinkR('Edit',"index.php?p=admin&c=bans&o=edit".$pagelink."&id=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']);

	$data['unban_link'] = CreateLinkR('Unban',"#","", "_self", false, "UnbanBan('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 1, false);return false;");
	$data['delete_link'] = CreateLinkR('Delete',"#","", "_self", false, "RemoveBan('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 0, false);return false;");
  ...
  $data['vacshow'] = !empty($data['steamid']);
	...
	if ($res->fields['demo_count'] == 0)
	{
		...
		$data['demo_quick'] = 'N/A';
		$data['demo_link'] = CreateLinkR('No demo',"#");
	}
	else
	{
		...
		if($commentres->RecordCount()>0) {
			...
			while(!$commentres->EOF) {
				...
				if($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
					$cdata['editcomlink'] = "<a href=\"index.php?p=banlist&comment=".$data['ban_id']."&ctype=B&cid=".$commentres->fields['cid'].$pagelink."\"> Edit</a>";
					if($userbank->HasAccess(ADMIN_OWNER)) {
						$cdata['delcomlink'] = "<a href=\"#\" target=\"_self\" onclick=\"RemoveComment(".$commentres->fields['cid'].",'B',".(isset($_GET["page"])?$page:-1).");\">Delete</a>";
					}
				}
				else {
...
if($pages > 1) {
	$ban_nav_p = ' / Pages: <div class="select" style="display: inline-block;"><select class="form-control" onchange="changePage(this,\'B\',\''.(isset($_GET['advSearch']) ? $_GET['advSearch'] : '').'\',\''.(isset($_GET['advType']) ? $_GET['advType'] : '').'\');" style="display: inline-block;width: 50px;">';
	for($i=1;$i<=$pages;$i++)
	{
