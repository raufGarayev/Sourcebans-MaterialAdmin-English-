...
function LoadServers2($check, $type, $length) {
	...
	if(!$userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN))
	{
		...
		$log = new CSystemLog("w", "Block try", $username . " tried to use block with no permission.");
		return $objResponse;
	}
	$id = 0;
...
function BlockPlayer($check, $sid, $num, $type, $length) {
	...
	{
		$objResponse->redirect("index.php?p=login&m=no_access", 0);
		$log = new CSystemLog("w", "Block try", $username . " tried edit block of player, with no permission.");
		return $objResponse;
	}
	...
	if($test = @fsockopen($sdata['ip'], $sdata['port'], $errno, $errstr, 2)) {
		...
		if(!$r->AuthRcon($sdata['rcon'])) {
			...
			$objResponse->addAssign("srv_$num", "innerHTML", "<font color='red' size='1'>Wrong RCON password, change it!</font>");
			$objResponse->addScript('set_counter(1);');
			return $objResponse;
		}
		$ret = $r->GetInfo();
		...
		if ($gothim) {
			...
			$objResponse->addAssign("srv_$num", "innerHTML", "<font color='green' size='1'><b>Player found and banned!</b></font>");
			$objResponse->addScript("set_counter('-1');");
			return $objResponse;
        }
		...
		$objResponse->addAssign("srv_$num", "innerHTML", "<font color='red' size='1'><i>Can not connect with server.</i></font>");
		$objResponse->addScript('set_counter(1);');
		return $objResponse;
	}
}
