...
{
    $log = new CSystemLog("w", "Hacking attempt", $userbank->GetProperty('user') . " tried to upload MOD icon with no permission.");
...
if(isset($_POST['upload']))
{
	if(CheckExt($_FILES['icon_file']['name'], "gif") || CheckExt($_FILES['icon_file']['name'], "jpg") || CheckExt($_FILES['icon_file']['name'], "png"))
	{
        ...
        $log = new CSystemLog("m", "MOD ico uploaded", "New MOD icon uploaded: ".htmlspecialchars($_FILES['icon_file']['name']));
	}
	else 
	{
		$message =  "<b> File format should be in gif, jpg or png.</b><br><br>";
	}
}
...
$theme->assign("formats", "GIF, PNG or JPG");
