...
while (!$res->EOF)
{
	...
	$info['popup'] = "ShowBox('Blocked player: " . $info['name'] . "', '" . $info['name'] . " tried join<br />' + document.getElementById('".$info['server']."').title + '<br />at " . $info['date'] . "<br /><div align=middle><a href=" . $info['search_link'] . ">Click to view details.</a></div>', 'red', '', true);";
...
while (!$res->EOF)
{
	...
	$info['created_info'] = SBDate("Issued ".$GLOBALS['config']['config.dateformat'],$res->fields['created']);
	$ltemp = explode(",",$res->fields[6] == 0 ? 'Permanently' : SecondsToString(intval($res->fields[6])));
	$info['length'] = $ltemp[0];
...
while (!$res->EOF)
{
	...
	$info['created'] = SBDate($GLOBALS['config']['config.dateformat_ver2'],$res->fields['created']);
	$info['created_info'] = SBDate("Issued ".$GLOBALS['config']['config.dateformat'],$res->fields['created']);
	$ltemp = explode(",",$res->fields[6] == 0 ? 'Permanently' : ($res->fields[6] < 0 ? "Session" : SecondsToString(intval($res->fields[6]))));
	$info['length'] = $ltemp[0];
	$info['icon'] = empty($res->fields[13]) ? 'web.png' : $res->fields[13];
