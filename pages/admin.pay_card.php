	...
	if(!$userbank->HasAccess(ADMIN_OWNER))
	{
		...
		if(($_GET['o'] == "del") && isset($_GET['o'])){
			if(!isset($_GET['id']) || !is_numeric($_GET['id'])){
				echo '<script>setTimeout(\'ShowBox("Error", "Ban ID not defined!", "red", "index.php");\', 1200);</script>';
				PageDie();
			}else{
				...
				if($qwer){
					...
					if($qww){
						echo '<script>setTimeout(\'ShowBox("Success", "Promokey successfully removed!", "green", "index.php?p=admin&c=pay_card");\', 1200);</script>';
					}
				}else{
					echo '<script>setTimeout(\'ShowBox("Error", "Ban ID not defined!", "red", "index.php");\', 1200);</script>';
				}
			...
			if(isset($_POST['pay_card_admin'])){
				if ($_POST['pay_card_admin'] == "pay_card_add"){
					if(($_POST['card_key'] != "") && ($_POST['card_exp'] >= 0) && ($_POST['card_gr_web'] != "")){
						...
						// err
						if(strlen($key_vr) <= 15){
							echo "<script>setTimeout(\"ShowBox('Promokey', 'Error, key should contain 16 symbols!', 'red', 'index.php?p=admin&c=pay_card#^1');\", 1200);</script>";
							...
							if($edit){
								echo "<script>setTimeout(\"ShowBox('Promokey', 'Promokey successfully added!', 'green', 'index.php?p=admin&c=pay_card');\", 1200);</script>";
							}else{
								echo "<script>setTimeout(\"ShowBox('Promokey', 'Error, can not add promokey!', 'red', 'index.php?p=admin&c=pay_card');\", 1200);</script>";
							}
						}else{
							echo "<script>setTimeout(\"ShowBox('Promokey', 'Error, current key already in system!', 'red', '', true);\", 1200);</script>";
						}
					}else{
						echo "<script>setTimeout(\"ShowBox('Promokey', 'Error, fill blanks correctly!', 'red', '', true);\", 1200);</script>";
					}
				}
			}
