...
function LoadServers($check) {
    ...
    if(!$userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN))
    {
        ...
        $log = new CSystemLog("w", "Hacking attempt", $username . " tried to use kick with no permission.");
        return $objResponse;
    }
    $id = 0;
...
function KickPlayer($check, $sid, $num) {
    ...
    {
        $objResponse->redirect("index.php?p=login&m=no_access", 0);
        $log = new CSystemLog("w", "Hacking attempt", $username . " tried edit kick of player with no permission.");
        return $objResponse;
    }
    ...
    if($test = @fsockopen($sdata['ip'], $sdata['port'], $errno, $errstr, 2)) {
        ...
        if(!$r->AuthRcon($sdata['rcon'])) {
            ...
            $objResponse->addAssign("srv_$num", "innerHTML", "<font color='red' size='1'>Error RCON password!</font>");
            $objResponse->addScript('set_counter(1);');
            return $objResponse;
        }
        $ret = $r->GetInfo();
        ...
        if (kickClient($r, $check)) {
            $objResponse->addAssign("srv_$num", "innerHTML", "<font color='green' size='1'><b>Found and kicked from server.</b></font>");
        ...
        $objResponse->addAssign("srv_$num", "innerHTML", "<font color='red' size='1'><i>No connection.</i></font>");
