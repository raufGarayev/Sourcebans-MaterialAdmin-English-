...
{
	echo '<script>ShowBox("Error", "Possible block try (URL key missmatching)!", "red", "index.php?p=admin&c=comms");</script>';
	PageDie();
}
if(!isset($_GET['id']) || !is_numeric($_GET['id']))
{
	echo '<script>ShowBox("Error", "No commblocks!", "red", "index.php?p=admin&c=comms");</script>';
	PageDie();
}
...
{
	echo '<script>ShowBox("Error", "You do not have access for this!", "red", "index.php?p=admin&c=comms");</script>';
	PageDie();
}
...
if(isset($_POST['name']))
{
	...
	{
		$error++;
		$errorScript .= "$('steam.msg').innerHTML = 'Type STEAM ID';";
	...
	if($error == 0)
	{
		...
		{
			$error++;
			$errorScript .= "$('steam.msg').innerHTML = 'This STEAM id already blocked';";
			$errorScript .= "$('steam.msg').setStyle('display', 'block');";
		}
		else
		{
	...
	if($error == 0)
	{
		...
		if($_POST['banlength'] != $lengthrev->fields['length'])
			$log = new CSystemLog("m", "Commsblock updated", "Comms block for (" . $lengthrev->fields['authid'] . ") has been updated, before: time ".$lengthrev->fields['length'].", type ".$lengthrev->fields['type']."; now: time ".$_POST['banlength']." type ".$_POST->fields['type']);
		echo '<script>ShowBox("Comms block updated", "Comms block updated successfully", "green", "index.php?p=commslist'.$pagelink.'");</script>';
	}
}

if(!$res)
{
	echo '<script>ShowBox("Error", "Error getting details. Maybe comms block already removed?", "red", "index.php?p=commslist'.$pagelink.'");</script>';
}
