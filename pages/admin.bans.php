...
global $userbank, $theme; if(!defined("IN_SB")){echo "Permssion error!";die();}if(isset($GLOBALS['IN_ADMIN']))define('CUR_AID', $userbank->GetAid());
		...
		if($line[1] == "0")
		{
			if(validate_ip($line[2])) // if its an banned_ip.cfg
			{
				...
				if($check->RecordCount() == 0)
				{
					...
					$GLOBALS['db']->Execute($pre, array("", $line[2], "Imported ban", 0, 0, "Import from banned_ip.cfg", $_COOKIE['aid'], $_SERVER['REMOTE_ADDR'], 1));
				}
				...
				if($check->RecordCount() == 0)
				{
					...
					$pre = $GLOBALS['db']->Prepare("INSERT INTO ".DB_PREFIX."_bans(created,authid,ip,name,ends,length,reason,aid,adminIp,type) VALUES
						(UNIX_TIMESTAMP(),?,?,?,(UNIX_TIMESTAMP() + ?),?,?,?,?,?)");
					$GLOBALS['db']->Execute($pre, array($steam, "", $pname, 0, 0, "Import from banned_user.cfg", $_COOKIE['aid'], $_SERVER['REMOTE_ADDR'], 0));
				}
			}
		}
	}
	if($bancnt > 0)
		$log = new CSystemLog("m", "Bans imported", "$bancnt Bans imported");

	echo "<script>setTimeout(\"ShowBox('Bans import', '$bancnt ban".($bancnt!=1?"s have":" has")." imported.', 'green', '', false, 5000)\", 800);</script>";
}
...
if((isset($_GET['action']) && $_GET['action'] == "pasteBan") && isset($_GET['pName']) && isset($_GET['sid'])) {
	echo "<script type=\"text/javascript\">setTimeout(\"ShowBox('Loading..','<i>Wait!</i>', 'blue', '', false, 5000);\", 800);xajax_PastePlayerData('".(int)$_GET['sid']."', '".addslashes($_GET['pName'])."');</script>";
}
...
<li id="utab-p0" class="active"><a href="index.php?p=admin&c=bans#^1~p0" id="admin_utab_p0" onclick="Swap2ndPane(0,\'p\');" class="tip"> Active </a></li> 
<li id="utab-p1" class="nonactive"><a href="index.php?p=admin&c=bans#^1~p1" id="admin_utab_p1" onclick="Swap2ndPane(1,\'p\');" class="tip"> Archieve </a></li> 
</ul>
</div>
</div>';
		// current protests
...
if ($page > 1)
{
	$prev = CreateLinkR('<- Previous',"index.php?p=admin&c=bans&ppage=" .($page-1). "#^1");
}
else
{
	$prev = "";
...
$page_nav = 'Showed&nbsp;'.$PageStart.'&nbsp;-&nbsp;'.$PageEnd.'&nbsp;from&nbsp;'.$page_count.'&nbsp;results';
...
foreach($protests as $prot)
{
	...
	array_push($protest_list, $prot);
		...
		if ($page > 1)
		{
			$prev = CreateLinkR('<- previous',"index.php?p=admin&c=bans&papage=" .($page-1). "#^1~p1");
		}
		else
		{
			$prev = "";
			...
			if($prot['archiv'] != "2") {
				...
				if(!$protestb) {
					...
					if(!$protestb[12])
						...
						$prot['server'] = $protestb[12];
					if($prot['archiv'] == "1")
						$prot['archive'] = "protest sent to archieve.";
					else if($prot['archiv'] == "3")
						$prot['archive'] = "ban expired.";
					else if($prot['archiv'] == "4")
						$prot['archive'] = "player unbanned.";
				}
			} else {
				$prot['archive'] = "ban removed.";
			}
			...
			if($commentres->RecordCount()>0) {
				...
				while(!$commentres->EOF) {
					...
					if($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
						$cdata['editcomlink'] = CreateLinkR('<img src=\'images/edit.gif\' border=\'0\' alt=\'\' style=\'vertical-align:middle\' />','index.php?p=banlist&comment='.(int)$prot['pid'].'&ctype=P&cid='.$commentres->fields['cid'],'Edit comment');
						if($userbank->HasAccess(ADMIN_OWNER)) {
							$cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.gif' border='0' alt='' style='vertical-align:middle' /> :: Remove comment\" target=\"_self\" onclick=\"Remove comment(".$commentres->fields['cid'].",'P',-1);\"><img src='images/delete.gif' border='0' alt='' style='vertical-align:middle' /></a>";
						}
					}
					else {
						$cdata['editcomlink'] = "";
	...
	//Submissions page
		...
		<a href="index.php?p=admin&c=bans#^2~s0" id="admin_utab_s0" onclick="Swap2ndPane(0,\'s\');" class="tip" title="List submissions :: List active submissions." target="_self">Active</a>
		</li>
		<li id="utab-s1" class="nonactive">
		<a href="index.php?p=admin&c=bans#^2~s1" id="admin_utab_s1" onclick="Swap2ndPane(1,\'s\');" class="tip" title="Show archieve :: Show submissions archieve." target="_self">Archieve</a>
		</li>
		</ul>
		</div>
		</div>';
		...
		if ($page > 1)
		{
			$prev = CreateLinkR('<- previous',"index.php?p=admin&c=bans&spage=" .($page-1). "#^2");
		}
		else
		{
			$prev = "";
		...
		foreach($submissions AS $sub)
		{
			...
			if(empty($sub['server']))
				$sub['hostname'] = '<i><font color="#677882">Another server...</font></i>';
			else
				$sub['hostname'] = "";
			...
			if($commentres->RecordCount()>0) {
				...
				while(!$commentres->EOF) {
					...
					if($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
						...
						if($userbank->HasAccess(ADMIN_OWNER)) {
							$cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" title=\"<img src='images/delete.gif' border='0' alt='' style='vertical-align:middle' /> :: Remove comment\" target=\"_self\" onclick=\"Remove comment(".$commentres->fields['cid'].",'S',-1);\"><img src='images/delete.gif' border='0' alt='' style='vertical-align:middle' /></a>";
						}
					}
					else {
						$cdata['editcomlink'] = "";
		...
		foreach($submissionsarchiv AS $sub)
		{
			...
			if(empty($sub['server']))
				...
				$sub['hostname'] = "";
			if($sub['archiv'] == "3")
				$sub['archive'] = "player banned.";
			else if($sub['archiv'] == "2")
				$sub['archive'] = "submission has been submitted.";
			else if($sub['archiv'] == "1")
				$sub['archive'] = "submission sent to archieve.";
				//COMMENT STUFF
				//-----------------------------------
			$view_comments = true;
			$commentres = $GLOBALS['db']->Execute(
		...
		<script type="text/javascript">
			...
			function ProcessBan()
			{
				...
				if(!$('nickname').value)
				{
					$('nick.msg').setHTML('Nick of player that you want to ban');
				...
				{
					$('steam.msg').setHTML('Type STEAM ID');
				...
				if(!reason)
				{
					$('reason.msg').setHTML('Choose reason of ban.');
			...
			function ProcessGroupBan()
			{
				if(!$('groupurl').value)
				{
					$('groupurl.msg').setHTML('Link of group that you ban');
