...
if(!$server)
{
	$log = new CSystemLog("e", "Could not get server details", "Can not find server details with identify '".$_GET['id']."'");
	echo '<div id="msg-red" >
	<i><img src="./images/warning.png" alt="Warning" /></i>
	<b>Error</b>
	<br />
...
if(isset($_POST['address']))
{
	...
	{
		$error++;
		$errorScript .= "$('address.msg').innerHTML = 'Type IP of server.';";
	...
	else
	{
		...
		{
			$error++;
			$errorScript .= "$('address.msg').innerHTML = 'Type valid IP.';";
			$errorScript .= "$('address.msg').setStyle('display', 'block');";
		}
	}
	...
	{
		$error++;
		$errorScript .= "$('port.msg').innerHTML = 'Type server port.';";
	...
	else
	{
		...
		{
			$error++;
			$errorScript .= "$('port.msg').innerHTML = 'Type valid port <b>number</b>.';";
			$errorScript .= "$('port.msg').setStyle('display', 'block');";
		}
	}
	
	// rcon
	if($_POST['rcon'] != '*Hided*' && $_POST['rcon'] != $_POST['rcon2'])
	{
		$error++;
		$errorScript .= "$('rcon2.msg').innerHTML = 'Passwords does not match.';";
	...
	if($error == 0)
	{
		...
		{
			$error++;
			$errorScript .= "ShowBox('Error', 'Server with this IP:PORT already exists.', 'red');";
		}
	}
	...
	if($_POST['rcon'] != '*Hided*')
	{
		...
		echo "<script>setTimeout(\"ShowBox('Server updated', 'Server details successfully updated', 'green', 'index.php?p=admin&c=servers');TabToReload();\", 1200);</script>";
	}
}
...
$theme->assign('rcon', 	'*Hided*'); // Mh, some random string
...
$theme->assign('submit_text', "Update details");
