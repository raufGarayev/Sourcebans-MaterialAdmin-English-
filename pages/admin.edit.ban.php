...
if($GLOBALS['config']['config.modgroup'] != "0"){
	...
	if($gid_groups == $GLOBALS['config']['config.modgroup']){
		...
		if($srv_ban != $amd_access){
			echo '<script>setTimeout(\'<script>ShowBox("Error", "You can edit bans on server where you have control permission!", "red", "");setTimeout(\'history.go(-1);\', 4000);\', 1200);</script>';
			PageDie();
		}
	}
}
...
{
	echo '<script>ShowBox("Error", "Possible block try (URL key missmatching)!", "red", "index.php?p=admin&c=bans");</script>';
	PageDie();
}
if(!isset($_GET['id']) || !is_numeric($_GET['id']))
{
	echo '<script>ShowBox("Error", "No ban!", "red", "index.php?p=admin&c=bans");</script>';
	PageDie();
}
...
{
	echo '<script>ShowBox("Error", "You do not have access for this!", "red", "index.php?p=admin&c=bans");</script>';
	PageDie();
}
...
if(isset($_POST['name']))
{
	...
	{
		$error++;
		$errorScript .= "$('steam.msg').innerHTML = 'You need to type STEAM ID';";
	...
	{
		$error++;
		$errorScript .= "$('steam.msg').innerHTML = 'Type valid STEAM ID';";
		$errorScript .= "$('steam.msg').setStyle('display', 'block');";
	}
	// Didn't type an IP
	...
	{
		$error++;
		$errorScript .= "$('ip.msg').innerHTML = 'Type IP Adress';";
		$errorScript .= "$('ip.msg').setStyle('display', 'block');";
	}
	else if ($_POST['type'] == 1 && !validate_ip($_POST['ip']))
	{
		$error++; 
		$errorScript .= "$('ip.msg').innerHTML = 'Type valid IP Adress';";
		$errorScript .= "$('ip.msg').setStyle('display', 'block');";
	}
	...
	if($demo_linker != ""){
		if(checkdnsrr($demo_linker_dns[0],'A') && @get_headers($demo_linker)){
			...
			$error++;
			$errorScript .= "$('demo_link.msg').innerHTML = 'Can not get server details! Advice: Read <img src=\"images/help.png\" />';";
	...
	{
		$error++;
		$errorScript .= "$('reason.msg').innerHTML = 'Type reason';";
	...
	if($error == 0)
	{
		...
		if($_POST['type'] == 0)
		{
			...
			{
				$error++;
				$errorScript .= "$('steam.msg').innerHTML = 'This STEAM ID already banned';";
				$errorScript .= "$('steam.msg').setStyle('display', 'block');";
			}
			else
			{
				...
				foreach($admchk as $admin)
				{
					...
					{
						$error++;
						$errorScript .= "$('steam.msg').innerHTML = 'Admin ".$admin['user']." have immunity';";
						$errorScript .= "$('steam.msg').setStyle('display', 'block');";
						break;
					}
				}
		...
		else if($_POST['type'] == 1)
		{
			...
			{
				$error++;
				$errorScript .= "$('ip.msg').innerHTML = 'This IP already banned';";
				$errorScript .= "$('ip.msg').setStyle('display', 'block');";
			}
		}
	}
	...
	if($error == 0)
	{
		...
		if($_POST['banlength'] != $lengthrev->fields['length'])
			$log = new CSystemLog("m", "Ban time changed", "Ban time (" . $lengthrev->fields['authid'] . ") has changed, before: ".$lengthrev->fields['length'].", now: ".$_POST['banlength']);
		echo "<script>setTimeout(\"ShowBox('Ban updated', 'Ban successfully updated', 'green', 'index.php?p=banlist".$pagelink."', false, 5000)\", 1000);</script>";
	}
}

if(!$res)
{
	echo "<script>setTimeout(\"ShowBox('Error', 'Error getting details. Maybe this ban already removed?', 'red', 'index.php?p=banlist".$pagelink."', false, 5000)\", 1000);</script>";
}
