...
{
	$log = new CSystemLog("w", "Block try", $userbank->GetProperty("user") . " tried edit group of admin".$userbank->GetProperty('user', $_GET['id']).". with no permision.");
	echo '<div id="msg-red" >
	<i><img src="./images/warning.png" alt="Warning" /></i>
	<b>Error</b>
	<br />
	You do not have access to edit groups.
</div>';
	PageDie();
}
...
{
	$log = new CSystemLog("e", "Could not get admin details", "Ca not find admin details with identify '".$_GET['id']."'");
	echo '<div id="msg-red" >
	<i><img src="./images/warning.png" alt="Warning" /></i>
	<b>Error</b>
	<br />
	Error getting current details.</div>';
	PageDie();
}
...
if(isset($_POST['wg']) || isset($_GET['wg']) || isset($_GET['sg']))
{
	...
	{
		echo '<script>setTimeout(function() { ShowBox("Error", "Admins need to have password and e-mail to get web permissions.<br /><a href=\"index.php?p=admin&c=admins&o=editdetails&id=' . $_GET['id'] . '\" title=\"Edit admin details\">Edit details</a> again or try later.", "red"); }, 1350);</script>';
	}
	else
	{
		...
		if(isset($GLOBALS['config']['config.enableadminrehashing']) && $GLOBALS['config']['config.enableadminrehashing'] == 1)
		{
			...
			echo '<script>setTimeout(\'ShowRehashBox("'.implode(",", $allservers).'", "Admin updated", "Admi details have been updated", "green", "index.php?p=admin&c=admins");TabToReload();\', 1350);</script>';
		}
		else
			echo '<script>setTimeout(\'ShowBox("Admin updated", "Admin details successfully updated", "green", "index.php?p=admin&c=admins");TabToReload();\', 1350);</script>';
		
		$admname = $GLOBALS['db']->GetRow("SELECT user FROM `".DB_PREFIX."_admins` WHERE aid = ?", array((int)$_GET['id']));
		$log = new CSystemLog("m", "Admin group updated", "Admin group of (" . $admname['user'] . ") updated");
	}
}
