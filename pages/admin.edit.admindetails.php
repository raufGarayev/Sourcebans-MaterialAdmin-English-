...
{
	$log = new CSystemLog("e", "Could not get admin details", "Ca not find admin details with indentify '".$_GET['id']."'");
	echo '<div id="msg-red" >
	<i><img src="./images/warning.png" alt="Warning" /></i>
	<b>Error</b>
	<br />
...
if(!$userbank->HasAccess(ADMIN_OWNER))
{
	...
	{
		$log = new CSystemLog("w", "Block try", $userbank->GetProperty("user") . " tried edit details ".$userbank->GetProperty('user', $_GET['id']).", with no permission.");
		echo '<div id="msg-red" >
		<i><img src="./images/warning.png" alt="Warning" /></i>
		<b>Error</b>
		<br />
		You do not have access to edit other profiles.
	</div>';
		PageDie();
	}
}
...
if(isset($_POST['adminname']))
{
	...
	if(!empty($_POST['period'])) {
		...
		if (!preg_match("|^[\d]+$|",$_POST['period'])) {
			$error++;
			$errorScript .= "$('period.msg').innerHTML = 'Only numbers';";
	...
	{
		$error++;
		$errorScript .= "$('adminname.msg').innerHTML = 'Type name of admin.';";
		...
		{
			$error++;
			$errorScript .= "$('adminname.msg').innerHTML = 'Admin name should not contain \" \' \".';";
			$errorScript .= "$('adminname.msg').setStyle('display', 'block');";
		}
		else
		{
            ...
            {
                $error++;
				$errorScript .= "$('adminname.msg').innerHTML = 'Admin with this name already exists.';";
				$errorScript .= "$('adminname.msg').setStyle('display', 'block');";
            }
		}
	}
	...
	{
		$error++;
		$errorScript .= "$('steam.msg').innerHTML = 'Type STEAM ID of admin.';";
	...
	else
	{
		...
		{
			$error++;
			$errorScript .= "$('steam.msg').innerHTML = 'Type real STEAM ID.';";
			$errorScript .= "$('steam.msg').setStyle('display', 'block');";
		}
		else
		{
			...
			if($a_steam != $userbank->GetProperty('authid', $_GET['id']) && is_taken("admins", "authid", $a_steam))
			{
				...
				$error++;
				$errorScript .= "$('steam.msg').innerHTML = 'Admin ".htmlspecialchars(addslashes($name))." already using this STEAM ID.';";
				$errorScript .= "$('steam.msg').setStyle('display', 'block');";
			}
		}
	}
	...
	if(empty($a_email))
	{
		...
		{
			$error++;
			$errorScript .= "$('email.msg').innerHTML = 'Type e-mail.';";
		...
		if($a_email != $userbank->GetProperty('email', $_GET['id']) && is_taken("admins", "email", $a_email))
		{
			...
			$error++;
			$errorScript .= "$('email.msg').innerHTML = 'This e-mail already in use ".htmlspecialchars(addslashes($name)).".';";
			$errorScript .= "$('email.msg').setStyle('display', 'block');";
		}
	...
	if($userbank->HasAccess(ADMIN_OWNER) || $_GET['id'] == $userbank->GetAid())
	{
		...
		if(!empty($_POST['password']))
		{
			...
			{
				$error++;
				$errorScript .= "$('password.msg').innerHTML = 'Your password should have minimum length " . MIN_PASS_LENGTH . " symbols.';";
			...
			else 
			{
				...
				{
					$error++;
					$errorScript .= "$('password2.msg').innerHTML = 'verify password.';";
				...
				{
					$error++;
					$errorScript .= "$('password2.msg').innerHTML = 'Passwords are not same!.';";
					$errorScript .= "$('password2.msg').setStyle('display', 'block');";
				}
			}
		}
		...
		if($_POST['a_useserverpass'] == "on")
		{
			...
			{
				$error++;
				$errorScript .= "$('a_serverpass.msg').innerHTML = 'Unchoose this section if password is not necessary.';";
			...
			{
				$error++;
				$errorScript .= "$('a_serverpass.msg').innerHTML = 'Your password should have minimum lenght " . MIN_PASS_LENGTH . " symbols.';";
				$errorScript .= "$('a_serverpass.msg').setStyle('display', 'block');";
			}
		}
	}
	...
	if($error == 0)
	{
		...
		$log = new CSystemLog("m", "Update admin details", "Admin (" . $admname['user'] . ") edit detail of admin");
		if($ownpwchanged)
			echo '<script>setTimeout(\'ShowBox("Updating details", "Updating details of admin...", "green", "index.php?p=admin&c=admins", true);TabToReload();\', 1200);</script>';
		else if(isset($rehashing))
			echo '<script>setTimeout(\'ShowRehashBox("'.implode(",", $allservers).'", "Updating details", " Updating details of admin...", "green", "index.php?p=admin&c=admins", true);TabToReload();\', 1200);</script>';
		else
			echo '<script>setTimeout(\'ShowBox("Updating details", "Updating details of admin...", "green", "index.php?p=admin&c=admins", true);TabToReload();\', 1200);</script>';
	}
}
...
else
{
	...
	if($a_expired == 0) {
		$a_expired_text = 'Never';
	}
	elseif($a_expired < time()) {
		$a_expired_text = 'Expired';
	}
	else{
		$a_expired_text = 'after&nbsp;'.round((($a_expired - time()) / 86400),0) . '&nbsp;days';
	}
	// ADM TIME //
}
