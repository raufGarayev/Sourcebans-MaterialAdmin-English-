...
if(!defined("IN_SB")){echo "Permission error!";die();}
	...
	if(isset($_GET['log_clear']) && $_GET['log_clear'] == "true")
	{
        ...
        {
            $log = new CSystemLog("w", "Hacking attempt", $userbank->GetProperty('user') . " tried to clear logs with no permission.");
        }
	}
...
else
{
	if(isset($_POST['settingsGroup']))
	{
		...
		if ($_POST['settingsGroup'] == "mainsettings_themes")
		{
			...
			?><script>setTimeout("ShowBox('Theme settings', 'Changes successfully saved!', 'green', 'index.php?p=admin&c=settings', false, 2500);", 1500);</script><?php 
		}
		if ($_POST['settingsGroup'] == "mainsettings")
		{
			if(!is_numeric($_POST['config_password_minlength']))
				$errors .= "Minimum password length<br />";
			if(!is_numeric($_POST['banlist_bansperpage']))
				$errors .= "Amount of bans in page, should be with numbers";
			if(empty($errors))
			{
				...
				?><script>setTimeout("ShowBox('Main settings changed', 'Changes successfully saved!', 'green', 'index.php?p=admin&c=settings', false, 2500);", 1200);</script><?php 
			}else{
		...
		if ($_POST['settingsGroup'] == "features")
		{
			...
			?><script>setTimeout("ShowBox('Settings changes saved', 'Changes successfully executed!', 'green', 'index.php?p=admin&c=settings');", 1200);</script><?php
		}
	}
	...
	echo '<div id="2" style="display:none;">';
		if($userbank->HasAccess(ADMIN_OWNER))
			$theme->assign('clear_logs', "( <a href='javascript:ClearLogs();'>Clear logs</a> )");
...
function MoreFields()
{
	...
	input_add.setAttribute("placeholder","Type details");
