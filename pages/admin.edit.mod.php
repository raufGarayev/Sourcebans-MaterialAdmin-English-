...
if(!isset($_GET['id']))
{
	echo '<script>ShowBox("Error", "No mods", "red", "", true);</script>';	
	PageDie();
}
if(!$userbank->HasAccess(ADMIN_OWNER|ADMIN_EDIT_MODS))
{
	$log = new CSystemLog("w", "Block try", $userbank->GetProperty("user") . " tried to edit Mod with no permission.");
	echo '<div id="msg-red" >
	<i><img src="./images/warning.png" alt="Warning" /></i>
	<b>Error</b>
	<br />
	You dont have access to edit MODs.
</div>';
	PageDie();
}
...
if(isset($_POST['name']))
{
	...
	{
		$error++;
		$errorScript .= "$('name.msg').innerHTML = 'Type Mod name.';";
	...
	else
	{
		...
		{
			$error++;
			$errorScript .= "$('name.msg').innerHTML = 'Mod with this name already exists.';";
			$errorScript .= "$('name.msg').setStyle('display', 'block');";
		}
	}
	...
	{
		$error++;
		$errorScript .= "$('folder.msg').innerHTML = 'Type Mod folder name.';";
	...
	else
	{
		...
		{
			$error++;
			$errorScript .= "$('folder.msg').innerHTML = 'This Mod folder already in use.';";
			$errorScript .= "$('folder.msg').setStyle('display', 'block');";
		}
	}
	...
	if($error == 0)
	{
		...
		echo '<script>ShowBox("Mod updated", "Mod successfully updated", "green", "index.php?p=admin&c=mods");</script>';
	}
...
if(!$res)
	echo '<script>ShowBox("Warning", "Error getting details. Maybe mod already removed?", "red", "index.php?p=admin&c=mod");</script>';
