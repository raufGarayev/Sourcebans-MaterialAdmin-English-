...
foreach($admins AS $admin)
{
	...
	if($admin['sk_profile'] == ""){
		$admin['sk_profile'] = "No Data";
	}else{
		$admin['sk_profile'] = $admin['sk_profile'] . " (<a href='skype:" . $admin['sk_profile'] . "?userinfo'>profile</a>)";
	}
	
	$admin['comment_profile'] = $userbank->GetProperty("comment", $admin['aid']);
	if($admin['comment_profile'] == ""){
		$admin['comment_profile'] = "No Comments.";
	}
  ...
  $admin['communityid_profile'] = CSteamId::factory(
	...
	{
  		$admin['web_group'] = "No permission";
	}
	if(empty($admin['server_group']) || $admin['server_group']==" ")
	{
		$admin['server_group'] = "No permission";
	}
	...
	if($admin['expired'] == 0) {
		$admin['expired_text'] = 'Never';
	}
	elseif($admin['expired'] < time()) {
		$admin['expired_text'] = 'Expired';
	}
	else{
		$admin['expired_text'] = 'After&nbsp;'.round((($admin['expired'] - time()) / 86400),0).'&nbsp;days.';
	}
	if($admin['expired'] == 0) {
		$admin['expired_cv'] = 'Permanently';
		$admin['del_link_d'] = 'if(confirm(\'This admin have permanent status.\nDo you really want to remove him?\')) { RemoveAdmin('.$admin['aid'].', \''.$admin['user'].'\'); }';
	}
	elseif($admin['expired'] < time()) {
		$admin['expired_cv'] = 'Already <b>Expired</b>';
		$admin['del_link_d'] = 'RemoveAdmin('.$admin['aid'].', \''.$admin['user'].'\');';
	} else {
		$admin['expired_cv'] = date('Till d.m.Y at <b>H:i</b>',$admin['expired']);
		$admin['del_link_d'] = 'if(confirm(\'This admin status is not expired.\nDo you really want to remove him?\')) { RemoveAdmin('.$admin['aid'].', \''.$admin['user'].'\'); }';
	}
...
if($pages > 1) {
	if (isset($_GET['showexpiredadmins']))
		$admin_nav_p = ' / Pages: <select class="form-control" onchange="window.location=\'index.php?p=admin&c=admins&showexpiredadmins=true&page=\' + $(\'PageChanger\').value;" style="display: inline-block;width: 40px;" id="PageChanger">';
	else
		$admin_nav_p = ' / Pages: <select class="form-control" onchange="changePage(this,\'A\',\''.$_GET['advSearch'].'\',\''.$_GET['advType'].'\');" style="display: inline-block;width: 40px;">';
...
if(isset($_GET["showexpiredadmins"]) && $_GET["showexpiredadmins"] == "true") {
	$btn_icon = "zmdi-alarm";
	$btn_helpa = 'data-trigger="hover" data-toggle="tooltip" data-placement="top" data-original-title="Show current admins" title=""';
	$btn_href = "index.php?p=admin&c=admins";
	$btn_rem = '<button type="button" onclick="removeExpiredAdmins()" class="btn bgm-bluegray btn-block waves-effect">Remove all expired admins</button>';
} else{
	$btn_icon = "zmdi-timer-off";
	$btn_helpa = 'data-trigger="hover" data-toggle="tooltip" data-placement="top" data-original-title="Show expired admins" title=""';
	...
	if(isset($_POST['new_override_name']))
	{
		...
		if(isset($_POST['override_id']))
		{
			...
			foreach($_POST['override_id'] as $index => $id)
			{
				...
				if(!empty($chk))
				{
					$edit_errors .= "&bull; This name already exists \\\"" . htmlspecialchars(addslashes($_POST['override_name'][$index])) . "\\\".<br />";
					continue;
				}
			...
			if(!empty($edit_errors))
				throw new Exception("Error on changes:<br /><br />" . $edit_errors);
		}
		...
		if(!empty($_POST['new_override_name']))
		{
			if($_POST['new_override_type'] != "command" && $_POST['new_override_type'] != "group")
				throw new Exception("Invalid override.");
			...
			if(!empty($chk))
				throw new Exception("Override with this name already exists.");
