...
if (isset($_GET['a']) && $_GET['a'] == "ungag" && isset($_GET['id']))
{
	...
	if($res){
		echo "<script>setTimeout('ShowBox(\"Player Ungagged\", \"Player <b>".StripQuotes($row['name'])."</b> (<b>" . $row['authid'] . "</b>) has been ungagged.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting..</b></font>\", \"green\", \"index.php?p=commslist$pagelink\", false);', 1350);</script>";
		$log = new CSystemLog("m", "Player Ungagged", "'".StripQuotes($row['name'])."' (" . $row['authid'] . ") has been ungagged");
	}else{
		echo "<script>setTimeout('ShowBox(\"Player is NOT ungagged\", \"There was an error ungagging <b>".StripQuotes($row['name'])."</b><br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=commsist$pagelink\", false);', 1350);</script>";
	}
}
else if(isset($_GET['a']) && $_GET['a'] == "unmute" && isset($_GET['id']))
{
	...
	if(empty($row) || !$row)
	{
		echo "<script>setTimeout('ShowBox(\"Player is NOT unmuted\", \"The player was not unmuted, either already unmuted or not a valid block.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=commslist$pagelink\", false);', 1350);</script>";
		PageDie();
	}
	...
	if($res){
		echo "<script>setTimeout('ShowBox(\"Player UnMuted\", \"Player <b>".StripQuotes($row['name'])."</b> <b>(" . $row['authid'] . ")</b> has been unmuted.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"green\", \"index.php?p=commslist$pagelink\", false);', 1350);</script>";
		$log = new CSystemLog("m", "Player Unmuted", "'".StripQuotes($row['name'])."' (" . $row['authid'] . ") has been unmuted");
	}else{
		echo "<script>setTimeout('ShowBox(\"Player is NOT unmuted\", \"There was an error unmuted <b>".StripQuotes($row['name'])."</b><br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>\", \"red\", \"index.php?p=commsist$pagelink\", false);', 1350);</script>";
	}
}
else if(isset($_GET['a']) && $_GET['a'] == "delete")
{
	...
	{
		echo "<script>setTimeout(\"ShowBox('Error', 'You do not have access for this.<br><br><font color=\'green\' class=\'f-15\'><b>Redirecting...</b></font>', 'red', 'index.php?p=commslist$pagelink');\", 1350);</script>";
		PageDie();
	}
...
// End custom entries
	...
	if($alrdybnd->fields['count']==0)
	{
		...
		{
		case 1:
			$data['reban_link'] = CreateLinkR('Remute',"index.php?p=admin&c=comms".$pagelink."&rebanid=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']."#^0");
			break;
		case 2:
			$data['reban_link'] = CreateLinkR('Regag',"index.php?p=admin&c=comms".$pagelink."&rebanid=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']."#^0");
			break;
		default:
			break;
		}
	...
	$data['edit_link'] = CreateLinkR('Edit',"index.php?p=admin&c=comms&o=edit".$pagelink."&id=".$res->fields['ban_id']."&key=".$_SESSION['banlist_postkey']);

	switch($data['type'])
	{
		case 3:
			$data['unban_link'] = CreateLinkR('Remove punishment (mute+gag)',"#","", "_self", false, "UnGag('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 1);return false;");
			break;
		case 2:
			$data['unban_link'] = CreateLinkR('Remove punishment (gag)',"#","", "_self", false, "UnGag('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 1);return false;");
			break;
		case 1:
			$data['unban_link'] = CreateLinkR('Remove punishment (mute)',"#","", "_self", false, "UnMute('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 1);return false;");
			break;
		default:
			break;
	}

	$data['delete_link'] = CreateLinkR('Delete',"#","", "_self", false, "RemoveBlock('".$res->fields['ban_id']."', '".$_SESSION['banlist_postkey']."', '".$pagelink."', '".StripQuotes($data['player'])."', 0);return false;");
		...
		if($commentres->RecordCount()>0) {
			...
			while(!$commentres->EOF) {
				...
				if($commentres->fields['aid'] == $userbank->GetAid() || $userbank->HasAccess(ADMIN_OWNER)) {
					...
					$cdata['editcomlink'] = CreateLinkR('Edit','index.php?p=commslist&comment='.$data['ban_id'].'&ctype=C&cid='.$commentres->fields['cid'].$pagelink);
					if($userbank->HasAccess(ADMIN_OWNER)) {
						$cdata['delcomlink'] = "<a href=\"#\" class=\"tip\" target=\"_self\" onclick=\"RemoveComment(".$commentres->fields['cid'].",'C',".(isset($_GET["page"])?$_GET["page"]:-1).");\">Remove</a>";
					}
				}
				else {
					$cdata['editcomlink'] = "";
